...
Conditions are tried in the order they are declared.
...
## Combining sub paths and conditional syntax

If you need to export multiple sub paths based on a condition, the `exports` field must use sub paths as top-level properties, and conditions inside each sub path. If the `exports` field uses conditions as top-level properties, `enhanced-resolve` will not be able to recognise sub paths further down in the object. In other words, only conditions may be nested inside a condition.

The following example is a valid way to combine sub paths and conditions, recognised by Node.js and webpack:
```json
{
  "exports": {
    "./sub/path": {
    ...
    "./other/path": {
      "browser": "./build/other-directory-with-polyfills/",
      "default": "./build/other-directory/"
    },
    ".": "./build/"
  }
}
...
The following example is NOT valid and will fail to work in webpack or Node.js:
```json
{
  "exports": {
    "development": {
      "./sub/path": "./src/directory/",
      "./other/path": "./build/other-directory/",
      ".": "./build/"
    },
    "browser": {
      "./sub/path": "./build/directory/",
      "./other/path": "./build/other-directory-with-polyfills/",
      ".": "./build/"
    },
    "default": {
      "./sub/path": "./build/directory/",
